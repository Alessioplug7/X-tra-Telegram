...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:  # pylint:disable=E0602
        ...
        msg = None
        message_to_reply = f"__❤️ciao non suo telegram da __ `{total_afk_time}`\nWhere He Is: ~~ONLY GOD KNOWS~~ " + \
            f"\n\n__non spammare altrimenti verrai bannato__\n**REASON**: {reason}" \
            if reason \
            else f"**Heya!**\n__I am currently unavailable. Since when, you ask? For {total_afk_time} I guess.__\n\nWhen will I be back? ~~Soon~~ __sei pregato di scrivere solo cosa ti serve❤️
